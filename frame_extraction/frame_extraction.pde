...
Rectangle selectedArea, prevArea;
...
void setup() {
  sprocketProcessor = new OpenCVPro(this, 500, 750);
  edgeProcessor = new OpenCVPro( this, 500, roiHeight );
  // delete ".DS_Store" file before running.
  File dir = new File("/Users/matthewepler/Documents/ITP/_THESIS/processing/kinograph-FrameProcessor/frame_extraction/data");
  String[] files = dir.list();
  if (dir.isDirectory()) 
  {
    for (File child : dir.listFiles()) 
    {
      if ( !child.getName().contains("Store") )
      {
        try
        {
          ...
          // of the frame
          float frameRight = 0.0;
          float frameLeft  = 0.0;
          for ( int i = 0; i < approximations.size(); i++ )
          {
            MatOfPoint2f m = approximations.get( i );
            float mx = (float)m.toArray()[0].x;
            if ( mx > 410 && mx < 420 )
            {
              frameRight = mx;
            } else if ( mx < 95 && mx > 85) {
          ...
          output.save( "output/" + child.getName().substring(0, 8) + ".jpg" );
          //image( output, 0, 0 );
          ...
          println( "* ERROR: " + child.getName() );
          selectedArea = new Rectangle( prevArea );
          // create a PImage for output and copy
          // the pixels from the source image into it.
          // NB: have to adjust the y-position down by the y-position of the ROI
          output = createImage(selectedArea.width, selectedArea.height, ARGB);
          output.copy(src, selectedArea.x, roi.y + selectedArea.y, selectedArea.width, selectedArea.height, 0, 0, selectedArea.width, selectedArea.height);
          String saveName = child.getName().substring( 0, 8 );
          output.save( saveName );
          println( counter + " of " + ( files.length - 1 ) + " CORRECTED!" );
          counter += 1;
          // convert edgeProcessor image into PImage for display (debugging only)
          //        dst2 = createImage(src.width, src.height, ARGB);
          //        edgeProcessor.toPImage(edgeProcessor.getBufferGray(), dst2);
